...
def get_n_classes(audio_path):
    ...
    n_classes = len(dirs) # M, F 이므로 len(dirs) = 2
...
# 오디오를 로드합니다. 파일이 아키텍처의 최대 용량보다 짧은 경우 오디오를 패딩으로 채우는 부분, 길다면 아키텍처 최대 길이만큼 자름
...
def create_dataset(audio_path, sample_rate, architecture_size, labels_saving_path):
    ...
    if architecture_size == 'audio_size':   # architecture_size : 오디오 길이
        audio_size_samples = 114688 # 모델 부분에서 strides 를 4로 지정하고 5초 이상의 파일 길이를 구함
